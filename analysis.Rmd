...
get_acs_data <- function(counties, state, min_year, max_year) {
  df <- map_df(silverline_counties, function(county) {
    get_acs(
      geography = "tract",
      variables = "B01003_001", # Total population
  ...
  for (i in (min_year+1):max_year) {
    temp_df <- map_df(silverline_counties, function(county) {
      get_acs(
        geography = "tract",
        variables = "B01003_001", # Total population
        state = state, # FIPS Code
...
pa_census_data <- get_acs_data(silverline_counties, "42", 2005, 2023)
```
...
generateFigures <- function(stations, map, yr_built, mid_distance=500, max_distance=10000) {
  ...
  mapStats <- map %>%
    ...
    summarize(hpi = mean(hpi),
              annual_change = mean(annual_change),
              pop = mean()) %>%
    ungroup()
  ...
  p3 <- ggplot(map) + 
    geom_point(aes(x=year, y=hpi, color=proximity), alpha = 0.5)+
...
generateFigures(northstar_stations, northstar_map, 2009, 500, 1000)
